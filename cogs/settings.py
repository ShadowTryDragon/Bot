...
class ServerSettings(commands.Cog):
    ...
    async def add_warn(self, guild_id, guild_name, user_id, user_name, reason):
        ...
        async with aiosqlite.connect("server_settings.db") as db:
            ...
            if result:
                ...
                warn_count = 1
                await db.execute(
                    "INSERT INTO warns (guild_id, guild_name, user_id, username, warn_count, last_warned) VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)",
    ...
    async def set_autorole(self, ctx,
                           ...
                           role2: Option(discord.Role, "OPTIMAL: Wähle eine weitere Rolle", required=False),
                           role3: Option(discord.Role, "OPTIMAL: Wähle eine weitere Rolle", required=False),
                           role4: Option(discord.Role, "OPTIMAL: Wähle eine weitere Rolle", required=False),
                           role5: Option(discord.Role, "OPTIMAL: Wähle eine weitere Rolle", required=False),
                           role6: Option(discord.Role, "OPTIMAL: Wähle eine weitere Rolle", required=False)):
        """Speichert die Standardrollen für neue Mitglieder."""
        roles = [role1]
        if role2: roles.append(role2)
        if role3: roles.append(role3)
        if role4: roles.append(role4)
        if role5: roles.append(role5)
        if role6: roles.append(role6)
   ...
   ### --- Warns --- ###
    ...
    async def on_message_edit(self, before, after):
        # Ignoriere Bot-eigene Nachrichten oder keine tatsächliche Änderung
        if before.author.bot or before.content == after.content:
            return

        # Hole Log-Channel-ID aus der DB
        async with aiosqlite.connect("server_settings.db") as db:
            db.row_factory = aiosqlite.Row
            async with db.execute("SELECT log_channel_id FROM settings WHERE guild_id = ?",
                                  (before.guild.id,)) as cursor:
                result = await cursor.fetchone()

        if not result or not result["log_channel_id"]:
            return

        log_channel = before.guild.get_channel(result["log_channel_id"])
        if not log_channel:
            return
        ...
        embed.add_field(name="Vorher", value=before.content[:1024] or "*Leerer Inhalt*", inline=False)
        embed.add_field(name="Nachher", value=after.content[:1024] or "*Leerer Inhalt*", inline=False)
    ...
    async def on_member_join(self, member):
        async with aiosqlite.connect("server_settings.db") as db:
            db.row_factory = aiosqlite.Row
            async with db.execute("SELECT * FROM settings WHERE guild_id = ?", (member.guild.id,)) as cursor:
    ...
    async def on_member_remove(self, member):
        ...
        embed = discord.Embed(
            title=settings["leave_message_header"],
            description=settings["leave_message"].replace("{user}", member.mention),
            color=int(settings["leave_message_color"].replace("#", ""), 16)
        )
        embed.set_footer(text=settings["leave_message_footer"])

        try:
            await channel.send(embed=embed)
        except Exception as e:
            print(f"⚠ Fehler beim Senden des Willkommens-Embeds: {e}")

    @commands.Cog.listener()
    async def on_message_delete(self, message):
        if message.author.bot:
            return  # Ignoriere Nachrichten vom Bot

        async with aiosqlite.connect("server_settings.db") as db:
            row = await db.execute("SELECT log_channel_id FROM settings WHERE guild_id = ?", (message.guild.id,))
        ...
        embed.set_footer(text=f"In Kanal: #{message.channel.name}")
        await log_channel.send(embed=embed)
