...
class TicketSystem(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        bot.ticket_system = self  # 🔹 Hier setzen wir die Referenz!
    ...
    async def update_ticket_embed(self, ticket_message: discord.Message, status: str):
        """Aktualisiert das Embed einer Ticket-Nachricht, wenn sich der Status ändert."""
        if not ticket_message.embeds:
            return  # Falls keine Embeds existieren, abbrechen

        embed = ticket_message.embeds[0]  # Erstes (und wahrscheinlich einziges) Embed abrufen

        # Status im Embed aktualisieren
...
class TicketArchiveConfirm(View):
    ...
    async def confirm_archive(self, button: Button, interaction: discord.Interaction):
        ...
        async with aiosqlite.connect("channels.db") as db:
            ...
            try:
                ticket_message = await self.channel.history().find(lambda m: m.author == self.bot.user and m.embeds)
                if ticket_message:
                    await self.bot.ticket_system.update_ticket_embed(ticket_message, "Archiviert")
